// src/pages/Grammie_PoppiePage.jsx
...
const Grammie_PoppiePage = () => {
  ...
  const photosRef = ref(storage, 'grammie_poppie_photos');

  // Settings States
  const [grammiePoppieClock, setGrammiePoppieClock] = useState(true);
  const [grammiePoppiePhoto, setGrammiePoppiePhoto] = useState(true);
  ...
  useEffect(() => {
      ...
      if (currentUser) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            // Check if 'grammie_poppie' is true
            if (userData.grammie_poppie) {
              setUser(currentUser);
    ...
    if (user) {
      const currentPhotoRef = doc(db, "settings", "grammie_poppie_settings");
  ...
  const fetchSettings = async () => {
    ...
    setSettingsError('');
    try {
      const settingsDocRef = doc(db, "settings", "grammie_poppie_settings");
      const settingsDoc = await getDoc(settingsDocRef);
      if (settingsDoc.exists()) {
        const settingsData = settingsDoc.data();
        setGrammiePoppieClock(settingsData.grammie_poppie_clock);
        setGrammiePoppiePhoto(settingsData.grammie_poppie_photo);
        setTimezone(settingsData.timezone || 'America/Los_Angeles'); // Set timezone
      } else {
        // If the document doesn't exist, create it with default values
        await setDoc(settingsDocRef, { grammie_poppie_clock: true, grammie_poppie_photo: true, timezone: 'America/Los_Angeles' });
        setGrammiePoppieClock(true);
        setGrammiePoppiePhoto(true);
  ...
  const handleToggleClock = async () => {
    const newValue = !grammiePoppieClock;
    setGrammiePoppieClock(newValue);
    await updateSettings("grammie_poppie_clock", newValue);
  };

  const handleTogglePhotos = async () => {
    const newValue = !grammiePoppiePhoto;
    setGrammiePoppiePhoto(newValue);
    await updateSettings("grammie_poppie_photo", newValue);
  };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, "settings", "grammie_poppie_settings"), (docSnapshot) => {
      if (docSnapshot.exists()) {
        const data = docSnapshot.data();
        setCurrentPhoto(data.current_photo || null);
      }
      ...
      {!user ? (
        ...
        <div className="photo-manager">
          <div className="header">
            <h2>Manage Grammie &amp; Poppie Photos</h2>
          ...
          {grammiePoppiePhoto && currentPhoto && (
...
export default Grammie_PoppiePage;